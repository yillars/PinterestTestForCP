...
static NSString *const CustomCollectionLayoutCellType   = @"CustomCollectionLayoutCellType";
NSString        *const CustomCollectionLayoutHeaderType = @"CustomCollectionLayoutHeaderType";
NSString        *const CustomCollectionLayoutFooterType = @"CustomCollectionLayoutFooterType";
...
# pragma mark - Lifecycle
  ...
  - ( void )setup {
      ...
      self.cellWidth         = (NSUInteger) ( [UIScreen mainScreen].bounds.size.width / 2 ); // Defaults to full width of iPhone + 10px gutters
  }
...
#pragma mark - UICollectionViewLayout methods
  ...
  - ( void )prepareLayout {
      ...
      for ( NSInteger section = 0; section < numSections; section++ ) {
          ...
          for ( NSInteger item     = 0; item < numItems; item++ ) {
              ...
              if ( indexPath.item == 0 && [delegate respondsToSelector:@selector(collectionView:layout:heightForHeaderInSection:)] ) {
                  UICollectionViewLayoutAttributes *headerAttributes = [self layoutAttributesForSupplementaryViewOfKind:CustomCollectionLayoutHeaderType atIndexPath:indexPath];
              ...
              if ( item == numItems - 1 && [delegate respondsToSelector:@selector(collectionView:layout:heightForFooterInSection:)] ) {
                  UICollectionViewLayoutAttributes *footerAttributes = [self layoutAttributesForSupplementaryViewOfKind:CustomCollectionLayoutFooterType atIndexPath:indexPath];
      ...
      newLayoutDictionary[ CustomCollectionLayoutCellType ]   = cellLayoutDictionary;
      newLayoutDictionary[ CustomCollectionLayoutHeaderType ] = headerLayoutDictionary;
      newLayoutDictionary[ CustomCollectionLayoutFooterType ] = footerLayoutDictionary;
  ...
  - ( NSArray * )layoutAttributesForElementsInRect:( CGRect )rect {
      ...
      [self.layoutInformation enumerateKeysAndObjectsUsingBlock:^(NSString *elementIdentifier, NSDictionary *elementsInfo, BOOL *stop) {
          ...
          [elementsInfo enumerateKeysAndObjectsUsingBlock:^(NSIndexPath *indexPath, UICollectionViewLayoutAttributes *layoutAttributes, BOOL *innerStop) {
              ...
              if ( CGRectIntersectsRect(rect, layoutAttributes.frame) || [elementIdentifier isEqualToString:CustomCollectionLayoutHeaderType] ) {
      ...
      [attributes enumerateObjectsUsingBlock:^(UICollectionViewLayoutAttributes *layoutAttributes, NSUInteger idx, BOOL *stop) {
          if ( layoutAttributes.representedElementKind == CustomCollectionLayoutHeaderType ) {
  ...
  - ( UICollectionViewLayoutAttributes * )layoutAttributesForSupplementaryViewOfKind:( NSString * )kind atIndexPath:( NSIndexPath * )indexPath {
      ...
      if ( kind == CustomCollectionLayoutHeaderType ) {
      ...
      if ( kind == CustomCollectionLayoutFooterType ) {
  ...
  - ( UICollectionViewLayoutAttributes * )initialLayoutAttributesForAppearingItemAtIndexPath:( NSIndexPath * )itemIndexPath {
      ...
      if ( [self.reloadIndexPaths containsObject:itemIndexPath] ) {
          attributes = self.layoutInformation[ CustomCollectionLayoutCellType ][ itemIndexPath ];
      } else {
